--[[ CHADLIX UNIVERSAL HUB - FULLY WORKING ]]
...
-- Tab Buttons
local tabs = {"Combat", "Movement", "Visuals", "Stats"}
local tabButtons = {}
...
for i, tabName in ipairs(tabs) do
    ...
    btn.Position = UDim2.new(0, 10 + (i-1)*118, 0, 50)
...
-- Content builder (doesn't clear toggle states)
...
function updateContent()
    ...
    if currentTab == "Combat" then
        makeToggle("Auto Clicker", 10, Settings.AutoClicker, function(on) 
            Settings.AutoClicker = on
            if on then startAutoClicker() else stopLoop("autoClicker") end
        end)
        makeToggle("Auto Shoot", 55, Settings.AutoShoot, function(on) 
            Settings.AutoShoot = on
            if on then startAutoShoot() else stopLoop("autoShoot") end
        end)
        makeToggle("Aim Assist", 100, Settings.AimAssist, function(on) 
            Settings.AimAssist = on
            if on then startAimAssist() else stopLoop("aimAssist") end
        end)
        makeSlider("Aim Strength", 150, 0.01, 1, Settings.AimStrength, function(v) Settings.AimStrength = v end)
        makeSlider("FOV Radius", 210, 20, 200, Settings.FOVRadius, function(v) Settings.FOVRadius = v end)
    
    elseif currentTab == "Movement" then
        makeToggle("Fly", 10, Settings.Fly, function(on) 
            Settings.Fly = on
            toggleFly()
        end)
        makeSlider("Fly Speed", 55, 10, 200, Settings.FlySpeed, function(v) Settings.FlySpeed = v end)
        makeToggle("Noclip", 115, Settings.Noclip, function(on) 
            Settings.Noclip = on
            toggleNoclip()
        end)
        makeToggle("Infinite Jump", 160, Settings.InfJump, function(on) 
            Settings.InfJump = on
            toggleInfJump()
        end)
    
    elseif currentTab == "Visuals" then
        makeToggle("Team ESP", 10, Settings.TeamESP, function(on) 
            Settings.TeamESP = on
            updateESP()
        end)
        makeToggle("Normal ESP", 55, Settings.NormalESP, function(on) 
            Settings.NormalESP = on
            updateESP()
        end)
        makeToggle("FPS / Ping", 100, Settings.FPSPing, function(on) 
            Settings.FPSPing = on
            fpsPingFrame.Visible = on
            if on then startFPSPing() else stopLoop("fpsPing") end
        end)
    ...
    elseif currentTab == "Stats" then
        ...
        for i, name in ipairs(statNames) do
            ...
            label.Position = UDim2.new(0, 10, 0, 10 + (i-1)*32)
...
function startFPSPing()
    stopLoop("fpsPing")
    loops.fpsPing = coroutine.wrap(function() updateFPSPing() end)
    loops.fpsPing()
end

-- Stats
function updateStats(statLabels)
    while true do
        ...
        if leaderstats then
            ...
            if kills and deaths and kills:IsA("IntValue") and deaths:IsA("IntValue") then
                local kd = deaths.Value > 0 and math.round(kills.Value / math.max(deaths.Value, 1) * 100) / 100 or kills.Value
            ...
            statLabels["💀 Kills"].Text = "💀 Kills: N/A"
            statLabels["☠️ Deaths"].Text = "☠️ Deaths: N/A"
            statLabels["📊 K/D Ratio"].Text = "📊 K/D Ratio: N/A"
            statLabels["⏱️ Time Played"].Text = "⏱️ Time Played: N/A"
        end
...
function startStats(statLabels)
    stopLoop("stats")
    loops.stats = coroutine.wrap(function() updateStats(statLabels) end)
    loops.stats()
end
...
function startAutoClicker()
    ...
    loops.autoClicker = coroutine.wrap(function() autoClickerLoop() end)
    loops.autoClicker()
end
...
function autoShootLoop()
    while Settings.AutoShoot do
        ...
        if target and target.Character and tool then
            ...
            -- Also try mouse click
            pcall(function() mouse1press() end)
            wait(0.01)
            pcall(function() mouse1release() end)
            
            -- Also try firing remote events in the tool
...
function startAutoShoot()
    stopLoop("autoShoot")
    loops.autoShoot = coroutine.wrap(function() autoShootLoop() end)
    loops.autoShoot()
end
...
function startAimAssist()
    stopLoop("aimAssist")
    loops.aimAssist = coroutine.wrap(function() aimAssistLoop() end)
    loops.aimAssist()
end
