--[[
    ...
    ALL ERRORS FIXED - AUTOSHOOT ADDED
]]
...
-- ==================== COMBAT TAB (UPDATED WITH AUTO SHOOT) ====================
local combatPage = tabPages["Combat"]

-- Auto Clicker Section
local acSection = CreateSection(combatPage, "Auto Clicker (Melee)")
...
CreateToggle(acSection, 30, "⚔️ Auto Clicker", false, function(enabled)
    ...
    if enabled then
        coroutine.wrap(autoClickerLoop)()
    else
        currentTarget = nil
    end
end)

-- Auto Shoot Section (NEW)
local asSection = CreateSection(combatPage, "Auto Shoot (Guns)")
asSection.Size = UDim2.new(1, 0, 0, 60)
CreateToggle(asSection, 30, "🔫 Auto Shoot", false, function(enabled)
...
combatPage.CanvasSize = UDim2.new(0, 0, 0, 290)
...
local function performClick()
    if currentTarget and currentTarget.Character then
        local targetPart = currentTarget.Character:FindFirstChild("Head") or 
                          currentTarget.Character:FindFirstChild("HumanoidRootPart") or
                          currentTarget.Character:FindFirstChild("UpperTorso") or
                          currentTarget.Character:FindFirstChild("Torso")
...
local function fireTool(tool)
    if tool then
        ...
        for _, remote in ipairs(tool:GetDescendants()) do
            if remote:IsA("RemoteEvent") and (remote.Name == "RemoteEvent" or remote.Name:lower():find("shoot") or remote.Name:lower():find("fire")) then
                remote:FireServer()
            end
        end
        
        -- Method 3: Simulate click events
        local clickDetectors = tool:GetDescendants()
...
function autoShootLoop()
    while Settings.AutoShoot do
        ...
        task.wait(0.07) -- Slightly longer delay to not spam too fast
    end
end
    ...
    while Settings.AimAssist do
        ...
        if target and target.Character then
            ...
            if head then
                ...
                local deltaX = (newPos.X - mousePos.X) / 2
                local deltaY = (newPos.Y - mousePos.Y) / 2
